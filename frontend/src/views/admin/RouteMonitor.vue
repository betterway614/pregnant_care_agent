...
const barOption = computed(() => ({
  ...
  series: [
    { name: '平均输入', type: 'bar' as const, data: agentData.value.map(d => d.avg_input_tokens), itemStyle: { color: '#3b82f6', borderRadius: [4, 4, 0, 0] } },
    { name: '平均输出', type: 'bar' as const, data: agentData.value.map(d => d.avg_output_tokens), itemStyle: { color: '#10b981', borderRadius: [4, 4, 0, 0] } },
  ],
}))
