<template>
  <div class="page-container">
    ...
    <!-- ==================== 健康趋势 ==================== -->
    <div ref="trendCardRef" class="patient-info-card">
      <div class="patient-info-card__title">
        <span class="dot" style="background: #42A5F5"></span>
        健康趋势
        <span v-if="trendLoading" class="status-badge status-badge--live" style="font-size: 10px">加载中</span>
      </div>
...
// 懒加载：进入视口时才请求数据
useIntersectionObserver(trendCardRef, ([{ isIntersecting }]) => {
  if (isIntersecting && !trendLoaded.value && pregnantId.value) {
    loadTrendData()
  }
})
...
function loadTrendData() {
  ...
  pregnantApi.getHealthTrends(pregnantId.value, {
...
function handleRangeChange(range: { startDate: string; endDate: string }) {
  if (!pregnantId.value || !selectedMetrics.value.length) return
  trendLoading.value = true
  pregnantApi.getHealthTrends(pregnantId.value, {
    metrics: selectedMetrics.value.join(','),
    axis_mode: trendAxisMode.value,
