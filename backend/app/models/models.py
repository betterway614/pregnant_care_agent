...
class Alert(Base):
    ...
    id = Column(UUIDColumn(as_uuid=True), primary_key=True, default=uuid.uuid4)
    pregnant_id = Column(String(64), ForeignKey("pregnant.pregnant_id"), nullable=False)
    trigger_source = Column(String(32), default="RULE_ENGINE", comment="RULE_ENGINE/FGR_ALGORITHM/MANUAL/EPDS_SCREENING/FOLLOWUP")
    ...
    status = Column(String(16), default="PENDING", comment="PENDING/CONFIRMED/DISMISSED/ESCALATED/AUTO_DISMISSED")
...
class AgentAuditLog(Base):
    ...
    tool_metrics_json = Column(JSON, nullable=True, comment="本次会话工具调用指标快照 ({tools:{name:{call_count,total_ms,avg_ms}},total_tool_calls,total_tool_ms})")
