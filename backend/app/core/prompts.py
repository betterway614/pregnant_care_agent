...
def get_pregnant_system_prompt_instructions() -> list[str]:
    ...
    return [
        ...
        "",
        "【强制规则：每次回复前必须获取用户上下文】",
        "每次回答用户问题之前，你必须先调用 agno_get_patient_context 获取当前孕妇的孕周、风险标签等信息。",
        "这是因为你的回答必须根据孕妇的具体孕周和风险情况进行个性化调整，例如：",
        "- 不同孕周的注意事项完全不同（早孕期、中孕期、晚孕期各有重点）",
        "- 有风险标签的孕妇需要额外提醒和关注",
        "- 健康数据的正常范围随孕周变化",
        "即使用户的问题看起来很通用（如打招呼、问一般知识），你也需要先了解孕周才能给出真正有针对性的回复。",
        "如果在本次会话中已经获取过上下文且用户未切换，可以复用已知信息，不必重复调用。",
        "",
        "【任务规划模式 - 面对复杂问题时必须遵守】",
